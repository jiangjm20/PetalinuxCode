...
static int axidma_probe(struct platform_device *pdev)
{
	...
	if (!lp->base_addr) {
		dev_err(dev, "axidma: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	if (!r_irq) {
		...
		dev_info(dev, "axidma at 0x%08x mapped to 0x%08x\n",
			(unsigned int __force)lp->mem_start,
			(unsigned int __force)lp->base_addr);
		return 0;
	}
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, &axidma_irq, 0, DRIVER_NAME, lp);
	if (rc) {
	...
	dev_info(dev,"axidma at 0x%08x mapped to 0x%08x, irq=%d\n",
		(unsigned int __force)lp->mem_start,
		(unsigned int __force)lp->base_addr,
		lp->irq);
	return 0;
