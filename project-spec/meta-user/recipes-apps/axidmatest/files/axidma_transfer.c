                         ...
                         char *output_path)
{
    ...
    axidma_oneway_transfer(dev,trans->input_channel,trans->input_buf,trans->input_size,true);
//    puts("Transmit");
...
//    puts("Receive");
    ...
    */
    ...
    printf("Time Elapsed: %d,%d to %d,%d\n",sts,stns,eds,edns);
/*    
    struct axidma_inout_transaction tr;

    // Setup the argument structure for the IOCTL
    tr.wait = 1;
    tr.tx_channel_id = trans->input_channel;
    tr.tx_buf = trans->input_buf;
    tr.tx_buf_len = trans->input_size;
    tr.rx_channel_id = trans->output_channel;
    tr.rx_buf = trans->output_buf;
    tr.rx_buf_len = trans->output_size;
*/

    // Copy in the video frame if it is specified
    // Perform the read-write transfer
//    rc = ioctl(dev->fd, AXIDMA_DMA_READWRITE, &tr);
...
//    rc = (rc < 0) ? -rc : 0;
    ...
    printf("Input,Output:%d,%d\n",trans.input_size,trans.output_size);
